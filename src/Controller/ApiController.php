<?php
/**
 * ApiController.php - Basket Api Controller
 *
 * Main Controller for Basket Api
 *
 * @category Controller
 * @package Basket
...
class ApiController extends CoreController {
    ...
    public function indexAction() {
        ...
        $aReturn = ['state'=>'success','message'=>'Welcome to onePlace Basket API'];
    ...
    public function listAction() {
        ...
        # Check license
        if(!$this->checkLicense('basket')) {
            $aReturn = ['state'=>'error','message'=>'no valid license for basket found'];
            echo json_encode($aReturn);
            return false;
        }
        ...
        foreach($aLangs as $sLoadLang) {
            if(file_exists(__DIR__.'/../../../oneplace-translation/language/'.$sLoadLang.'.mo')) {
                $translator->addTranslationFile('gettext', __DIR__.'/../../../oneplace-translation/language/'.$sLang.'.mo', 'basket', $sLoadLang);
            }
        }
        ...
        $aFields = $this->getFormFields('basket-single');
        $aFieldsByKey = [];
        ...
        # Get All Basket Entities from Database
                        ...
                        switch($oField->type) {
                            case 'multiselect':
                                ...
                                foreach($oTags as $oTag) {
                                    $aTags[] = ['id'=>$oTag->id,'label'=>$translator->translate($oTag->text,'basket',$sLang)];
                                }
                            ...
                            case 'select':
                                ...
                                if($oTag) {
                                    if (property_exists($oTag, 'tag_value')) {
                                        $aPublicItem[$oField->fieldkey] = ['id' => $oTag->id, 'label' => $translator->translate($oTag->tag_value,'basket',$sLang)];
                                    } else {
                                        $aPublicItem[$oField->fieldkey] = ['id' => $oTag->getID(), 'label' => $translator->translate($oTag->getLabel(),'basket',$sLang)];
                                    }
                                }
                                break;
                            case 'text':
                            case 'date':
                            case 'textarea':
                                $aPublicItem[$oField->fieldkey] = $translator->translate($oItem->getTextField($oField->fieldkey),'basket',$sLang);
                                break;
                            default:
                                break;
                        }
    ...
    public function getAction() {
        ...
        try {
            ...
            $aReturn = ['state'=>'error','message'=>'Basket not found','oItem'=>[]];
            echo json_encode($aReturn);
            return false;
        }

        # Print Entity
        $aReturn = ['state'=>'success','message'=>'Basket found','oItem'=>$oItem];
        echo json_encode($aReturn);
